...
export default function DirectProcurement() {
  ...
  const { data: bomItems = [], isError, error } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/bom-items/${selectedBomId}`),
              ...
              {recentOrders.map((order: any) => (
                <Card key={order.id} className="border-2 hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="px-6 pb-0">
                    ...
                    <div className="flex gap-2 pt-4 border-t px-0 pb-2">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => {
                      ...
                      {(order.status === 'draft' || order.status === 'pending_approval') && (
                        <Button
                          ...
                          className="flex-1 border-2 h-9 text-red-600 hover:text-red-700 hover:bg-red-50"
