...
export class DatabaseStorage implements IStorage {
  ...
  async getAuctions(filters: any = {}): Promise<Auction[]> {
    ...
    return query.orderBy(desc(auctions.createdAt));
  }
  ...
  async getAuctionParticipants(auctionId: string): Promise<AuctionParticipant[]> {
    return db.select().from(auctionParticipants).where(eq(auctionParticipants.auctionId, auctionId));
  }

  async createAuctionParticipant(participant: InsertAuctionParticipant): Promise<AuctionParticipant> {
    const [result] = await db.insert(auctionParticipants).values(participant).returning();
    return result;
  }
