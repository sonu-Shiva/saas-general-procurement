...
// GST Master table for tax calculations
export const gstMasters = pgTable("gst_masters", {
  ...
  status: varchar("status", { enum: ["active", "inactive", "draft"] }).default("active").notNull(),
  notes: text("notes").default(""),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  createdBy: varchar("created_by").default("system").notNull(),
  updatedBy: varchar("updated_by").default("system").notNull(),
}, (table) => [
  unique("unique_hsn_effective_from").on(table.hsnCode, table.effectiveFrom),
]);

// GST Master insert schema with validations
